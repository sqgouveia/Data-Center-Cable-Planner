...
function routeBetweenRacks(aId,bId,c){
  const temp={...c,originRack:aId,destRack:bId,via:[]};
...
function calcCable(c){
  ...
  if(manual){
    if(md.reachable&&md.segments.length){
      const first=md.segments[0],last=md.segments[md.segments.length-1];
      const tempFirst={...c,originRack:c.originRack,destRack:(c.via?.[0]||c.destRack),via:[]};
      const tempLast={...c,originRack:(c.via?.[c.via.length-1]||c.originRack),destRack:c.destRack,via:[]};
      const rf=calcAutomaticTrayLength(tempFirst),rl=calcAutomaticTrayLength(tempLast);
      v1=rf.reachable?rf.length:0; v2=rl.reachable?rl.length:0; tray=Math.max(0,md.length-v1-v2);
    }
...
function bind(){
  ...
  const setSidebarCollapsed=(collapsed,persist=true)=>{
    ...
    // Keep the racks at the exact same screen coordinates when the sidebar changes width.
    // The canvas viewport itself moves horizontally with the grid column, so compensate
    // the pan by the opposite amount after the layout transition.
    const beforeLeft=wrap?.getBoundingClientRect().left ?? 0;
    const pan=window.__canvasPan;
    const beforeX=pan?.x ?? 0;
    ...
    requestAnimationFrame(()=>{
      if(wrap&&pan){
        const afterLeft=wrap.getBoundingClientRect().left;
        pan.x=beforeX+(beforeLeft-afterLeft);
        window.__applyCanvasPan?.();
      }
      window.__updateMinimap?.();
    });
  };
  if(sidebarToggle){
  ...
  $('btnQuickSearch')?.addEventListener('click',openSearch);
  $('quickSearchClose')?.addEventListener('click',closeQuickSearch); $('summaryClose')?.addEventListener('click',closeProjectSummary);
  $('quickSearchModal')?.addEventListener('click',e=>{if(e.target.id==='quickSearchModal')closeQuickSearch();}); $('projectSummaryModal')?.addEventListener('click',e=>{if(e.target.id==='projectSummaryModal')closeProjectSummary();});
  ...
  window.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'){e.preventDefault();openQuickSearch();}else if(e.key==='Escape'){if($('quickSearchModal')?.classList.contains('open'))closeQuickSearch();else if($('projectSummaryModal')?.classList.contains('open'))closeProjectSummary();}});
