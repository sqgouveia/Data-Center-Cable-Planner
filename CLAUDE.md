...
- `rack-metrics.js` — pure per-rack numbers (U used per face and combined, power/weight vs capacity, warranty/EOL counts) behind the canvas "Colorir por" heat layers, the rack hover tooltip and the room summary shown in the properties panel when nothing is selected. Like `occupancy.js` it takes racks and assets as arguments, so it is unit-tested (`js/test/rack-metrics.test.mjs`). The chosen layer is a per-browser preference (`dc-planner-heat-mode` in localStorage), not part of the project.
- `plant-export.js` — exports the canvas drawing as SVG/PNG ("Exportar planta" next to the layer control) and provides the scale-1:N first page of the PDF report. It clones the live `#layout` SVG, inlines each element's computed style (the exported file has no access to `app.css`), crops to the drawing and always renders with the light theme so the file prints well. Scale bar and the 1:N choice use `geometry().scale` (px per metre). Pure helpers (`niceScale`, `composePlantSvg`, …) are unit-tested; the DOM capture is not.
- `breakout-model.js` — pure model of the MTP breakout cable: lane from the port letter (`1A`…`1H`), supplier products `{ m: total, leg, price? }` and the pick of the smallest that fits, the leg-as-virtual-cable (`breakoutLegCable`, which is how port occupancy and route drawing see breakouts) and the import grouping. `calcBreakout` (trunk/leg from the real routes) lives in `routing.js`. Both unit-tested (`breakout-model.test.mjs`, `breakout-calc.test.mjs`).
- `breakouts.js` — the "Breakouts" tab of the Cables card, the breakout properties panel, "+ Breakout", and the summary/export rows. Breakouts live per room in `state.breakouts` (a `ROOM_KEYS` entry); types in `state.cableCatalogs.breakoutTypes`. `configureBreakouts({...})`.
